...
const fattoInCasaDaBenedetta = (url) => {
  ...
  return new Promise((resolve, reject) => {
    if (!url.includes('fattoincasadabenedetta.it/')) {
      reject(
        new Error("url provided must include 'fattoincasadabenedetta.it/'")
      );
    } else {
      request(url, (error, response, html) => {
        if (!error && response.statusCode === 200) {
          ...
          Recipe.name =
            rawRecipeName[0].toUpperCase() + rawRecipeName.substring(1);
          ...
          $('.recipe-time-box').each((i, el) => {
            $(el)
              ...
              .each((i, el) => {
                ...
                const data = $(el).find('.time-text + span').text();
          ...
          Recipe.servings = $('.advanced-adjust-recipe-servings').attr(
            'data-original'
          );
