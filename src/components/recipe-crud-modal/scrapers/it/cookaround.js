    ...
    if (!url.includes('cookaround.com/')) {
      ...
      request(url, (error, response, html) => {
        if (!error && response.statusCode === 200) {
          ...
          let originalName = $("meta[property='og:title']").attr('content');
          let originalSiteName = $("meta[property='og:site_name']").attr(
            'content'
          );

          Recipe.name = originalName.replace(originalSiteName, '');

          $('.r-ingredients, .recipe-ingredients-content').each((i, el) => {
            $(el)
              .find('li')
              .each((i, elChild) => {
            ...
            $(el)
              ...
              .each((i, elChild) => {
                Recipe.instructions.push(
                  $(elChild).text().replace(/\s\s+/g, ' ').trim()
                );
              });
          });
          ...
          if ('blog' === subDomains[0]) {
            ...
            Recipe.time.prep = $('.preptime').find('.recipe-value').text();

            Recipe.time.cook = $('.cooktime').find('.recipe-value').text();
          } else {
