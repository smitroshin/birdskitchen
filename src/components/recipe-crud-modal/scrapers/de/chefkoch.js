...
const chefKoch = (url) => {
  ...
  return new Promise((resolve, reject) => {
    if (!url.includes('chefkoch.de/rezepte')) {
      reject(new Error("url provided must include 'chefkoch.de/rezepte/'"));
    } else {
      request(url, (error, response, html) => {
        if (!error && response.statusCode === 200) {
          ...
          Recipe.name = $("meta[property='og:image:alt']").attr('content');
          ...
          Recipe.servings = $('input[name="portionen"]').attr('value');
