    ...
    if (!url.includes('daskochrezept.de/')) {
      ...
      request(url, (error, response, html) => {
        if (!error && response.statusCode === 200) {
            ...
            $(el)
              ...
              .each((i, elChild) => {
                $(elChild)
                  .find('i')
                  .each((i, elGrandChild) => {
                    const title = $(elGrandChild).attr('class');
                    if (title.includes('icon--preparation')) {
                    ...
                    if (
                      title.includes('icon--cooking-time') ||
          ...
          Recipe.servings = $('.portion-calculator__portions').text().trim();
