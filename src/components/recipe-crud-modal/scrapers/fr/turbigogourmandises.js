...
const turbigoGourmandises = (url) => {
  ...
  return new Promise((resolve, reject) => {
    if (!url.includes('turbigo-gourmandises.fr/')) {
      reject(new Error("url provided must include 'turbigo-gourmandises.fr'"));
    } else {
      request(url, (error, response, html) => {
        if (!error && response.statusCode === 200) {
          ...
          $('.wprm-recipe-prep-time-container').each((i, el) => {
            Recipe.time.prep = $(el).find('.wprm-recipe-time').text().trim();
          });

          $('.wprm-recipe-cook-time-container').each((i, el) => {
            Recipe.time.cook = $(el).find('.wprm-recipe-time').text().trim();
          });
...
export default turbigoGourmandises;
