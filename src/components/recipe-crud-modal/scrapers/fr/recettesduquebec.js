...
const recettesDuQuebec = (url) => {
  const Recipe = new RecipeSchema();
  return new Promise((resolve, reject) => {
    if (!url.includes('recettes.qc.ca/')) {
      reject(new Error("url provided must include 'recettes.qc.ca'"));
    } else {
          ...
          $('.recipe-infos').each((i, el) => {
            $(el)
              ...
              .each((i, elChild) => {
                ...
                switch (title) {
                  case 'Préparation':
                    Recipe.time.prep = $(elChild).find('.value').text();
                    break;
                  case 'Cuisson':
                    Recipe.time.cook = $(elChild).find('.value').text();
                    break;
                  case 'Portion(s)':
...
export default recettesDuQuebec;
