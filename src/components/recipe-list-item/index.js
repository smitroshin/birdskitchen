...
class RecipeListItemNotExtended extends Component {
      ...
      buttons: [
        {
          ...
          onClick: () => {
            ...
            setTags();
            NotyHelpers.open(
              feather.icons.trash.toSvg() + t('This recipe has been trashed!'),
              'error',
              2000
            );
          },
  ...
  onClickRemovePermanently = (id) => {
    ...
    openConfirmDialog({
      title: t('Confirmation'),
      text: t(
        'Do you really want to delete this recipe permanently? This process cannot be undone!'
      ),
      buttons: [
        {
          label: t('Yes'),
          onClick: () => {
            ...
            setTags();
            NotyHelpers.open(
              feather.icons.zap.toSvg() +
                t('This recipe has been deleted permanently!'),
              'info',
              1500
            );
          },
  ...
  restoreFromTrash = (item) => {
    ...
    NotyHelpers.open(
      feather.icons.award.toSvg() +
        t('The recipe has been restored from trash!'),
      'success',
      2000
    );
  };
  ...
  getTags = () => {
    ...
    let tags =
      '' === item.tags || null === item.tags ? [] : item.tags.split(',');
  ...
  render() {
    ...
    return (
      <div className="comp_recipe-list-item">
        ...
        <div
          onClick={() => this.setState({ showGeneratorModal: true })}
        ...
        >
          ...
          <div className="right-side">
            {'trash' !== selectedMenu.slug && item.isfavorite ? (
              <SvgIcon name="bookmark" />
            ) : null}
          </div>
        </div>
        ...
        {'trash' === selectedMenu.slug ? (
          <ul className="recipe-list-menu">
            <li
              className="trash"
              title=""
              onClick={() => this.onClickRemovePermanently(item.id)}
            >
              <SvgIcon name="trash" />
            </li>
            <li
              className="restore"
              title={t('Restore')}
              onClick={() => this.restoreFromTrash(item)}
            >
          ...
          <ul className="recipe-list-menu">
            ...
            >
              {item.isfavorite ? (
                ...
                <SvgIcon name="bookmark_outline" />
              )}
            </li>
            <li
              className="edit"
              title={t('Edit ')}
              onClick={() => this.setState({ showCrudModal: true })}
            >
              <SvgIcon name="edit" />
            </li>
            <li
              onClick={() => this.onClickMoveOnTrash(item)}
              className="trash"
...
const RecipeListItem = hoistStatics(
  withTranslation()(RecipeListItemNotExtended),
  RecipeListItemNotExtended
);
