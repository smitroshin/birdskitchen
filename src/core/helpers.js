...
const ReduxHelpers = {
  ...
  fillCategories: (dispatch) =>
    dispatch({
      type: SET_CATEGORIES,
      payload: CategoriesHelpers.getAllItems(),
    }),

  fillRecipes: (dispatch, selectedMenu, query) =>
    dispatch({
...
const StorageHelpers = {
  ...
  readImg: (file) => {
    if (undefined !== file) {
      const mediasDir =
        storagePreferences.get('storagePath').toString() + '/medias/';
  ...
  moveDb: (willMoveDir) => {
    const oldPathForDbFile = path.join(
      storagePreferences.get('storagePath').toString(),
      App.dbName
    );
    const newPathForDbFile = path.join(willMoveDir, App.dbName);
    const mediasFolderExistPath =
      storagePreferences.get('storagePath').toString() + '/medias';
  ...
  backupNow: () => {
    ...
    const dbBackupDir = path.join(
      storagePreferences.get('backupPath').toString(),
